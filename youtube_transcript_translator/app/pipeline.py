...
from ..normalize.regroup import regroup_subtitles
...
def run_pipeline(config: PipelineConfig, *, target_dir: Path | None = None) -> PipelineResult:
    ...
    if config.input_path is not None:
        ...
        english_segments, input_reference = resolve_transcript_from_url(
            ...
            transcription_backend=config.transcript.backend,
            transcription_model=config.transcript.openai_model,
            ...
            openai_api_key_env=config.transcript.openai_api_key_env,
            transcription_timeout_seconds=config.transcript.timeout_seconds,
            transcription_chunk_seconds=config.transcript.chunk_seconds,
    ...
    write_srt(output_path, translated_segments)
    ...
    if config.output.json_output is not None:
        write_segments_json(config.output.json_output.resolve(), translated_segments)
